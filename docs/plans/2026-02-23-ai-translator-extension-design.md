...
**範例**：
```
選取："The ephemeral nature of social media"
↓
📖 翻譯
   社群媒體的短暫性質
...
💡 說明
   這個片語描述社群媒體內容容易消失、不持久的特性。
   其中 "ephemeral" 是關鍵詞，意思是「短暫的」。

✨ 例句
   • The ephemeral nature of social media makes
     it hard to preserve memories.
     (社群媒體的短暫性質使得保存記憶變得困難)
```
...
```
選取："The ephemeral nature of social media posts means
       they are easily forgotten and rarely archived."
↓
📖 翻譯
   社群媒體貼文的短暫性意味著它們很容易被遺忘，
   也很少被存檔。
```
...
| `Let's go.` | ✅ | 2 | 翻譯 🌐 |
| `What is this?` | ✅ | 3 | 翻譯 🌐 |
| `The ephemeral nature of social media` | ❌ | 6 | 學習 🎓 |
| `The ephemeral nature of social media posts means they are easily forgotten` | ❌ | 13 | 翻譯 🌐 |
| `曖昧な表現` | ❌ | 1 | 學習 🎓 |
| `これは曖昧な表現です。` | ✅ | 1 | 翻譯 🌐 |
...
3. **請求翻譯**
   ...
   - Content Script 分析選取文字，判斷模式
   - Content Script 發送訊息給 Background Script
   - Background Script 根據模式呼叫 API
